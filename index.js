...
const SUPPORTED_IDENTITY_REQUESTS_PER_HOUR = process.env.SUPPORTED_IDENTITY_REQUESTS_PER_HOUR;
...
function processIdentify(client, data) {
    ...
    nodeWallet.signMessage(data.slice(20)).then((signature) => {
        client.ws.send((0, ethers_1.getBytes)((0, ethers_1.concat)([client.authMessage, signature])));
    });
}
function processAuthentication(client, data) {
    if ((0, ethers_1.verifyMessage)(client.authMessage, (0, ethers_1.hexlify)(data)).toLowerCase() !==
        client.identity) {
...
const isEligibleForRequest = (commit = false) => {
    ...
    if (hourlyRequestCount > SUPPORTED_IDENTITY_REQUESTS_PER_HOUR) {
        return false;
    }
    if (commit)
        requests.push(now);
